...
def extract_json(raw_output: str) -> dict:
    ...
    # 2. Sanitize the JSON string
    # The original regex was flawed. A simple state-based parser is more reliable
    # for escaping newlines only *inside* string literals.
    
    sanitized_chars = []
    in_string = False
    # We need to track the previous character to correctly handle escaped quotes
    # For example, we don't want to toggle `in_string` on a \" character.
    last_char = '' 
    ...
    for char in json_str:
        ...
        # If we encounter a newline while inside a string, escape it.
        # Otherwise, keep it, as it might be for formatting the JSON structure.
        if char == '\n' and in_string:
    ...
    # 3. Parse the sanitized string using json5
    try:
